...
class TopDealsTable extends Component {
  checkIfOnSteam(name, shop_url, to_display) {
    ...
    if(temp === undefined){
        return <a style={{ color: "white" }} href={shop_url}>{to_display}</a>
    } else {
        return <Link style={{ color: "white" }} to={`/app/${temp}`}>{to_display}</Link>;
    }
  }
          ...
          {this.props.apps.map((app, index) => (
            <tr key={this.props.getIdByName(app.title)}>
              ...
              <td className={"align-middle table-borderless"} style={{ width: "8%" }}>
                {this.checkIfOnSteam(app.title, app.urls.buy, <SearchResultsData appid={this.props.getIdByName(app.title)} />)}
              </td>
              ...
              <td className={"align-middle"} style={{ width: "10%" }}>
                <a className={"btn btn-success"} style={{padding: "0.25rem"}} href={app.urls.buy}>{app.shop.name}</a>
              </td>
            </tr>
          ))}
        </tbody>
